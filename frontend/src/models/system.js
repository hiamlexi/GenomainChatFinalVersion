...
const System = {
  cacheKeys: {
    footerIcons: "genomain_footer_links",
    supportEmail: "genomain_support_email",
    customAppName: "genomain_custom_app_name",
    canViewChatHistory: "genomain_can_view_chat_history",
    deploymentVersion: "genomain_deployment_version",
  },
  ping: async function () {
  ...
  requestToken: async function (body) {
    // Check if we should use gateway endpoint for multi-user mode
    const isMultiUser = await this.isMultiUserMode().catch(() => false);
    const endpoint = isMultiUser ? "/gateway/request-token" : "/request-token";
